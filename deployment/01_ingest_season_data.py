...
# MARKDOWN CELL ********************
# # 🏎️ 1. Ingest Season Data (Bronze)
# 
# **Description:** This notebook fetches raw data from the OpenF1 API and saves it as JSON files in the Lakehouse (Bronze Layer).
# 
# **Prerequisites:**
# 1. Add a Lakehouse to this notebook.
# 2. Set it as the **Default Lakehouse** (Pin icon 📌).
# 
# **Parameters:**
# * `YEAR`: The F1 season to ingest (Default: "2023").
...
# Enable dynamic partition overwrite to update data without deleting the whole history
spark.conf.set("spark.sql.sources.partitionOverwriteMode", "dynamic")

BASE_URL = "https://api.openf1.org/v1"
# Relative path - Works with any attached default Lakehouse
...
# Note: This might fail during live F1 sessions due to API restrictions on free tier.
try:
    races_raw = requests.get(f"{BASE_URL}/sessions?year={YEAR}&session_type=Race").json()
    
    # Check if API returned an error (dict) instead of a list
    if isinstance(races_raw, dict) and 'detail' in races_raw:
        raise Exception(f"API Error: {races_raw['detail']}")
    ...
    for meeting in meetings_sorted:
        ...
        # FILTER: Skip Testing sessions to focus on Race Weekends
        ...
        try:
            # ==========================================
            # 1. GENERAL DOWNLOADS (With pauses to respect API limits)
            # ==========================================
            ...
            # B. Download Results for each valid session (Race & Sprint)
    ...
    print("If you see a 'Session in progress' error, please wait until the live F1 session concludes.")
